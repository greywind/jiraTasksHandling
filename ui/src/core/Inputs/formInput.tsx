...
const useHorizontalStyles = createUseStyles(
    {
        wrapper: {
            ...
            flexDirection: "row",
            width: "100%",
            margin: (p: Partial<SharedFormProps>) =>
                p.autoMargin ? "0 0 10px 0" : "unset",
        },
        title: {
            minWidth: (p: Partial<SharedFormProps>) => p.titleWidth || "unset",
            padding: (p: Partial<SharedFormProps>) =>
                p.autoMargin ? "6px 10px 0px 0px" : "unset",
            whiteSpace: "nowrap",
        },
        content: {
            width: "100%",
...
const HorizontalLayout: React.FunctionComponent<LayoutProps> = ({
    ...
    const classes = useHorizontalStyles({
        ...formMethods,
        autoMargin: autoMarginValue,
        titleWidth: titleWidth || formMethods.titleWidth || "unset",
    });
    return (
        <div className={classnames(classes.wrapper, "wrapper")}>
            ...
            <div className={classnames(classes.content, "content", type)}>
                {content}
            </div>
            <If condition={!!error}>
                <div className={classnames(classes.error, "error")}>
                    <Tooltip
                        position="bottom"
                        html={<Alert color="danger">{error.message}</Alert>}
                    >
                        <FontAwesomeIcon
...
const useVerticalStyles = createUseStyles(
    {
        ...
        header: {
            ...
            alignItems: "center",
            padding: (p: Partial<SharedFormProps>) =>
                p.autoMargin ? "8px 0 5px 3px" : "unset",
            fontSize: "13px",
            lineHeight: "18px",
        },
        title: {
    ...
    return (
        <div className={classnames(classes.wrapper, "wrapper")}>
            <div className={classnames(classes.header, "header")}>
                ...
                <If
                    condition={
                        formMethods.errors[name] && !!formMethods.errors[name].message
                    }
                >
                    <div className={classnames(classes.error, "error")}>
                        {formMethods.errors[name] && formMethods.errors[name].message}
...
const FormInput: React.FunctionComponent<FormInputProps> = ({
    ...
    const onFileChangeProxy = useCallback(
        (_: string, value: FileInfo) => {
            setValue(name, value?.id || null);
        },
        [name, setValue]
    );
    const onFilesChangeProxy = useCallback(
        (_: string, value: FileInfo[]) => {
        ...
        switch (type) {
            case "date":
                result.validate = (data: number) => {
                    if (max && moment(data) > moment(max.value)) return max.message;
                    if (min && moment(data) < moment(min.value)) return min.message;
                    if (required && isNaN(data)) return required;
                    return true;
                };
                break;
            case "number":
                result.validate = (data: number) => {
                    if (max && +data > +max.value) return max.message;
                    if (min && +data < +min.value) return min.message;
                    if (required && isNaN(data)) return required;
                    return true;
                };
                break;
    ...
    const proxyBooleanValue = useMemo(() => {
    ...
    return (
        <>
            <If condition={type != "hidden"}>{layoutElement}</If>
            <input
                type="hidden"
                name={name}
                value={value}
                ref={formMethods.register(validationProxy)}
            />
        </>
    );
};
