...
const columnStyle = (column: Column<unknown>): any => {
  ...
  if (column.width) result.minWidth = column.width;
  return result;
};
...
declare let column: Column<unknown>;
declare let rowValue: unknown;
...
const CustomControls: React.FunctionComponent<CustomControlsProps> = props => {
  ...
  return (
    <div className={classes.container}>
      <div>
        <If condition={!!props.customControls}>{props.customControls}</If>
      </div>
      <div>
        <If condition={!!props.page && !!props.pageCount}>pagination</If>
      </div>
    </div>
  );
};
...
const Body: React.FunctionComponent<TableProps<unknown>> = props => {
  ...
  const onRowClick = React.useCallback(
    (value: unknown, i: number) => {
      if (propsOnRowClick) propsOnRowClick(value, i);
    },
    [propsOnRowClick]
  );
  return (
      ...
      <For each="rowValue" of={props.values} index="i">
        <tr
          ...
          className={classnames({
            [classes.selected]: rowValue == props.selectedValue,
          })}
          onClick={() => onRowClick(rowValue, i)}
        >
          <For each="column" of={props.columns}>
            <td style={columnStyle(column)} key={`${i}_${column.title}`}>
              <Cell column={column} value={rowValue} index={i} />
            </td>
          </For>
        </tr>
      </For>
...
function Table<T>(props: TableProps<T>): React.ReactElement | null {
  ...
  const tableClassName = useMemo(() => (classes as any)["table-" + view], [
    classes,
    view,
  ]);
  const customTop = useMemo(
    () =>
      !props.customControlsPosition || props.customControlsPosition == "top",
    [props.customControlsPosition]
  );
  const customBottom = useMemo(
    () =>
      !props.customControlsPosition || props.customControlsPosition == "bottom",
    [props.customControlsPosition]
  );
  return (
    <div className={classes.container}>
...
export default function createTable<T>(): React.FunctionComponent<
  TableProps<T>
> {
