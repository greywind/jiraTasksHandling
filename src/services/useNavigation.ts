...
const links = {
    home: "/",
    page1: "/page1",
    page2: { link: "/page2/:param", regex: /^\/page2\/[^/]+$/, build: (name: string) => `/page2/${name}` },
};
...
export function useNavigation<T = RouterParams>() {
    ...
    const result = React.useMemo(() => {
        let currentPage: Page = Object.entries<Link>(links).find(([, value]) => {
        ...
        return {
            ...
            to: (page: Page, ...args: string[]) => {
                ...
                if (typeof target === "string")
                    ...
                    history.push(target.build.apply(null, args));
            },
        };
