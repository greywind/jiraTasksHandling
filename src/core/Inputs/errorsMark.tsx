...
const ErrorsMark: React.FunctionComponent<ErrorsMarkProps> = props => {
    ...
    const regex = new RegExp(props.pattern || ".*", "gi");
    const errors = Object.keys(formMethods.errors).filter(
        p => regex.test(p) && formMethods.errors[p].message
    );
    const htmlContent = useMemo(
        () => <Alert color="danger">{errors.join(",")}</Alert>,
        [errors]
    );
    return (
        <>
