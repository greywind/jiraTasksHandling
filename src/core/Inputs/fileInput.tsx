...
declare var file: FileInfo;

const useStyles = createUseStyles({
    dropZone: {
        border: (p: Partial<NoFileViewProps>) => `dashed 1px ${p?.error ? theme.current.colors.danger : theme.current.colors.regularBorder}`,
    ...
    filesContainer: {
        border: (p: Partial<FilesInputProps>) => `solid 1px ${p?.error ? theme.current.colors.danger : theme.current.colors.regularBorder}`,
        padding: 5,
    },
}, { name: "fileInput" });
...
const NoFileView: React.FunctionComponent<NoFileViewProps> = ({ onDrop, error, innerRef }) => {
    const classes = useStyles({ error });
    return <Dropzone preventDropOnDocument onDrop={onDrop}>{({ getRootProps, getInputProps }) => {
        return <div {...getRootProps()} className={classes.dropZone}>
            <div className={classnames({ [classes.dropZoneContent]: true })}>
...
const UploadingFileView: React.FunctionComponent<UploadingFileViewProps> = props => {
    ...
    return <div className={classes.dropZoneUploading}>
...
const FilePreview: React.FunctionComponent<FilePreviewProps> = ({ file, wrapperClassName }) => {
    if (file.type == "Image") {
    ...
    return <div className={wrapperClassName}>
        ...
        Size: <b>{readableDataSize(file.size)}</b><br />
...
const FileView: React.FunctionComponent<FileViewProps> = ({ file, onCancel }) => {
    const classes = useStyles();
    const download = useCallback((): void => {
        if (file)
            fileSvc.download(file.id);
    }, [file]);
    const onClose = useCallback(() => onCancel(file?.id), [file?.id, onCancel]);
    return <div className={classes.dropZoneContentExists}>
        <Button className={classes.linkButton} onClick={download} color="link">
            <Tooltip position="bottom" html={<FilePreview file={file} wrapperClassName={classes.imagePrevirew} />}>
                {file?.name}
            </Tooltip>
        </Button>
        <div onClick={onClose}>
...
export const FileInput: React.FunctionComponent<FileInputProps> = ({ name, value, onChange, error, fileId, innerRef }) => {
    ...
    const onDrop = useCallback(async (files: File[]) => {
        ...
        const result = await fileSvc.upload(files[0], p => setProgress((p.loaded * 100) / p.total));
        setStatus("File");
        setFile(result);
...
export const FilesInput: React.FunctionComponent<FilesInputProps> = ({ name, value, onChange, error, filesId }) => {
    const classes = useStyles({ error });
    ...
    useEffect(() => {
        (async () => {
            if (value) {
                ...
                const ids = typeof filesId === "string" ? filesId.split(",").map(p => p.trim()) : filesId;
    ...
    const onFileUplaod = useCallback((name: string, file: FileInfo) => {
    ...
    return <div className={classes.filesContainer}>
