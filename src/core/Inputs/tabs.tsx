...
import CarouselWithPreview, { CarouselWithPreviewPropsControls } from "@shared/carouselWithPreview";
...
declare var panel: React.Component<SshPanelProps>;
declare var i: number;
declare var tabTitle: ReactNode;
declare var navTab: SshNavTabRoute;
...
export const SshPanel: React.FunctionComponent<SshPanelProps> = props => {
    ...
    return <div className={`${props.wrapperClassName} ${props.className}`} style={styles}>
...
const SshTabsHeader: React.FunctionComponent<SshTabsHeaderProps> = props => {
    const useStyles = props.layout == "vertical" ? useVerticalStyles : props.layout == "separateTabs" ? useSeparateTabsLayout : useHorizontalStyles;
    ...
    if (screenSize.let("sm") && props.layout == "separateTabs") {
        return <CarouselWithPreview interval={false} activePageChanged={props.tabClick} height={50} controls={controls}>
            <For each="tabTitle" of={props.tabs} index="i">
                <div
                    key={(typeof tabTitle === "string" && tabTitle) ? tabTitle : i}
                    className={classnames(classes.tabsHeader, { [classes.active]: i == props.activeTab })}>
                    {tabTitle}
                </div>
            </For>
    ...
    return <div className={classes.tabHeaderContainer}>
...
export const SshTabs: React.FunctionComponent<SshTabsProps> = props => {
    const useStyles = props.layout == "vertical" ? useVerticalStyles : props.layout == "separateTabs" ? useSeparateTabsLayout : useHorizontalStyles;
    const classes = useStyles();
    const childrenArray = React.Children.toArray(props.children).filter(p => React.isValidElement<SshPanelProps>(p) && p.type == SshPanel) as React.Component<SshPanelProps>[];
    const footer = React.Children.toArray(props.children).find(p => React.isValidElement<SshTabsFooterProps>(p) && p.type == SshTabsFooter);
    ...
    return <>
        <div className={classnames(props.wrapperClassName, classes.containerWrapper)}>
            <SshTabsHeader
                activeTab={activeTab}
                ...
                <div className={classes.content}>
                    <For each="panel" of={childrenArray} index="i">
                        <div
                            style={tabStyle}
                            onClick={() => tabClick(i)}
                            className={classnames({ [classes.hidden]: i != activeTab })}>
...
export const SshNavTabs: React.FunctionComponent<SshNavTabsProps> = props => {
    ...
    const classes = useStyles();
    const tabClick = useCallback((i: number) => {
        if (props.tabs[i].route)
            return history.push(props.tabs[i].route);
    ...
    const routes = props.tabs.map(tab => tab.route || links[tab.page] as string);

    return <div className={classnames(props.wrapperClassName, classes.containerWrapper)}>
        <SshTabsHeader
            activeTab={activeTab}
            customControl={props.customControl}
            layout={props.layout}
        ...
        <div className={classnames(props.contentClassName, classes.content)}>
            <Switch>
                <For each="navTab" of={props.tabs} index="i">
                    <Route key={routes[i]} path={routes[i]} component={navTab.component} exact={navTab.exact} />
                </For>
