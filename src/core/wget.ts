...
class Wget {
    ...
    private async request(method: string, action: string, body: any, opt: any, retries: number = 4): Promise<any> {
        opt = opt || {};
        opt.responseType = opt.responseType || "json";
        opt.qs = opt.qs || {};
        if (this.siteId)
            opt.qs.siteId = this.siteId;
        if (this.rayId)
            opt.qs.rayId = this.rayId;
        const qs = queryString.stringify(flattenObject(opt.qs) as ParsedUrlQueryInput);

        const composedUrl = `${configSvc.value.apiUrl}/${action}${qs ? `?${qs}` : ""}`;
        function logError(message: string, e: Error): void {
            Logger.network(message, { url: `${window.location.protocol}${composedUrl}`, method, action, body, opt, retries, exception: e });
        }
        let response: Response;
        try {
            const args: RequestInit = { method, credentials: "include", headers: {} };
            if (body)
            ...
            if (e instanceof TypeError) {
                Logger.network(e.message, { url: `${window.location.protocol}${composedUrl}`, method, action, body, opt, retries, response, exception: e });
                if (retries) {
                    // eslint-disable-next-line no-console
                    console.warn(`Failed to make request to ${method}:${action}`, body, opt, retries);
                    await delay(500);
                    return this.request(method, action, body, opt, retries - 1);
                }
                throw { message: (localeSvc.get("common") as LocaleCommon).tryAgainLater, type: "critical", skipLoggingIfNotCatched: true };
            }
            if (response && response.status >= 400 && response.status < 500)
                Logger.metrica("wget4xx", e.message, { method, action, opt, retries, status: response.status });
            else
...
function resolveReferences(json: any): any {
    ...
    let byid: any = {}; // all objects by id
    let refs: any[] = []; // references to objects that could not be resolved
